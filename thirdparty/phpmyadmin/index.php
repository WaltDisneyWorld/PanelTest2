...
if ($server > 0 || count($cfg['Servers']) > 1
     ...
     */
    if (0 == $cfg['ServerDefault']
        || (!$cfg['NavigationDisplayServers']
        && (count($cfg['Servers']) > 1
        || (0 == $server && 1 == count($cfg['Servers']))))
    ) {
...
if (false == $cfg['LoginCookieValidityDisableWarning']) {
    /**
 ...
 */
if (0 != $GLOBALS['cfg']['LoginCookieStore']
 ...
 */
if (false == $cfg['SuhosinDisableWarning']
    && ini_get('suhosin.request.max_value_length')
    && '0' == ini_get('suhosin.simulation')
) {
    trigger_error(
        sprintf(
            __(
