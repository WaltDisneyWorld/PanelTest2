...
if (isset($_POST['format']) && 'ldi' == $_POST['format']) {
...
if (isset($_POST['console_bookmark_add'])) {
    if (isset($_POST['label']) && isset($_POST['db'])
        ...
        $isShared = ('true' == $_POST['shared'] ? true : false);
        ...
        if (false !== $bookmark && $bookmark->save()) {
...
if ('table' == $import_type) {
    $goto = 'tbl_import.php';
} elseif ('database' == $import_type) {
    $goto = 'db_import.php';
} elseif ('server' == $import_type) {
...
if ('import.php' === basename($_SERVER['SCRIPT_NAME'])) {
...
if (Encoding::isSupported() && isset($charset_of_file)) {
    if ('utf-8' != $charset_of_file) {
        $charset_conversion = true;
    }
} elseif (isset($charset_of_file) && 'utf-8' != $charset_of_file) {
...
if (!empty($id_bookmark) && 2 == $_POST['action_bookmark']) {
    $message = PhpMyAdmin\Message::success(__('The bookmark has been deleted.'));
    $display_query = $import_text;
    $error = false; // unset error marker, it was used just to skip processing
} elseif (!empty($id_bookmark) && 1 == $_POST['action_bookmark']) {
...
if ($timeout_passed) {
    ...
    if (0 == $offset || (isset($original_skip) && $original_skip == $offset)) {
        $message->addText(
            __(
