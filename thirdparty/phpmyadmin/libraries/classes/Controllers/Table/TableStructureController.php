...
class TableStructureController extends TableController
{
    ...
    public function indexAction()
    {
        ...
        if (isset($_POST['reserved_word_check'])) {
            if (false === $GLOBALS['cfg']['ReservedWordDisableWarning']) {
                ...
                if (0 == count($reserved_keywords_names)) {
    ...
    protected function moveColumns()
    {
        ...
        for ($i = 0, $l = count($_POST['move_columns']); $i < $l; ++$i) {
            ...
            if (isset($data['Extra'])
                && 'on update CURRENT_TIMESTAMP' == $data['Extra']
            ) {
                $extracted_columnspec['attribute'] = $data['Extra'];
                unset($data['Extra']);
            }
            $current_timestamp = ('timestamp' == $data['Type']
                    || 'datetime' == $data['Type'])
                && ('CURRENT_TIMESTAMP' == $data['Default']
                    || 'current_timestamp()' == $data['Default']);

            if ('YES' === $data['Null'] && null === $data['Default']) {
                $default_type = 'NULL';
            } elseif ($current_timestamp) {
                $default_type = 'CURRENT_TIMESTAMP';
            } elseif (null === $data['Default']) {
                $default_type = 'NONE';
            } else {
    ...
    private function _extractPartitionDetails()
    {
        ...
        $partitionDetails['can_have_subpartitions']
            = $partitionDetails['partition_count'] > 1
                && ('RANGE' == $partitionDetails['partition_by']
                || 'RANGE COLUMNS' == $partitionDetails['partition_by']
                || 'LIST' == $partitionDetails['partition_by']
                || 'LIST COLUMNS' == $partitionDetails['partition_by']);
    ...
    protected function displayStructure(
        ...
        if (true === $GLOBALS['cfg']['HideStructureActions']) {
