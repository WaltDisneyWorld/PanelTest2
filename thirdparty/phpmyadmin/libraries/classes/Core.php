...
class Core
{
    ...
    public static function isValid(&$var, $type = 'length', $compare = null)
    {
        ...
        // do the check
        if ('length' === $type || 'scalar' === $type) {
            $is_scalar = is_scalar($var);
            if ($is_scalar && 'length' === $type) {
                return strlen($var) > 0;
            }
            return $is_scalar;
        }
 ...
 // end function
    ...
    public static function fatalError($error_message, $message_args = null)
    {
         ...
         */
        if (!is_null($GLOBALS['dbi']) && isset($GLOBALS['PMA_Config']) && false === $GLOBALS['PMA_Config']->get('is_setup') && Response::getInstance()->isAjax()) {
 ...
 // end getRealSize()
    ...
    public static function downloadHeader($filename, $mimetype, $length = 0, $no_cache = true)
    {
        ...
        if (false !== strpos($mimetype, 'gzip') && $notChromeOrLessThan43) {
    ...
    public static function arrayRemove($path, array &$array)
    {
        ...
        for (; $depth >= 0; --$depth) {
            if (!isset($path[$depth + 1]) || 0 == count($path[$depth + 1])) {
 ...
 // end of the 'getIp()' function
    ...
    {
        while ('p:' == strtolower(substr($name, 0, 2))) {
            $name = substr($name, 2);
        }
