...
class Util
{
 ...
 // end of the 'showPHPDocu()' function
    ...
    private static function _checkRowCount($db, array $table)
    {
        ...
        if (null === $table['Rows']) {
            ...
            $tbl_is_view = 'VIEW' == $table['TABLE_TYPE'];
 ...
 // end of the 'backquoteCompat()' function
    ...
    public static function getMessage(
        ...
        $render_sql = true == $cfg['ShowSQL'] && !empty($sql_query) && ';' !== $sql_query;
 ...
 // end of the 'formatByteDown' function
    ...
    public static function formatNumber(
        ...
        if (0 === $digits_left) {
            ...
            if ((0 != $originalValue) && (0 == floatval($value))) {
        ...
        if ($noTrailingZero && false !== strpos($formattedValue, $decimal_sep)) {
...
// end of the 'extractValueFromFormattedSize' function
    ...
    public static function localisedDate($timestamp = -1, $format = '')
    {
        ...
        if ('UTF-8' != mb_detect_encoding($ret, 'UTF-8', true)) {
 ...
 // end of the 'localisedDate()' function
    ...
    public static function getHtmlTab(array $tab, array $url_params = array())
    {
        ...
        $item['class'] = 'active' == $tab['class'] ? 'active' : '';
 ...
 // end of the 'getHtmlTab()' function
    ...
    public static function linkOrButton(
        ...
        if (!empty($target)) {
            ...
            if ('_blank' === $target && 0 == strncmp($url, 'url.php?', 8)) {
        ...
        if (($url_length > $GLOBALS['cfg']['LinkLengthLimit'])
            ...
            if (array_key_exists('class', $tag_params)
                && false !== strpos($tag_params['class'], 'create_view')
            ) {
 ...
 // end function
    ...
    public static function getUniqueCondition(
        ...
        for ($i = 0; $i < $fields_cnt; ++$i) {
            ...
            if (!isset($meta->orgname) || 0 === strlen($meta->orgname)) {
            ...
            if (!isset($row[$i]) || is_null($row[$i])) {
                ...
                if ($meta->numeric
                    ...
                    $con_val = '= '.$row[$i];
                } elseif ((('blob' == $meta->type) || ('string' == $meta->type))
 ...
 // end function
    ...
    public static function getButtonOrImage(
        ...
        if ('text' == $GLOBALS['cfg']['ActionLinksMode']) {
 ...
 // end function
    ...
    public static function getListNavigator(
        ...
        $class = 'frame_navigation' == $frame ? ' class="ajax"' : '';
    ...
    public static function getExternalBug(
        ...
        $ext_but_html = '';
        if (('mysql' == $component) && ($GLOBALS['dbi']->getVersion() < $minimum_version)) {
    ...
    public static function getDivForSliderEffect($id = '', $message = '', $overrideDefault = null)
    {
        return Template::get('div_for_slider_effect')->render([
            'id' => $id,
            'initial_sliders_state' => (null != $overrideDefault) ? $overrideDefault : $GLOBALS['cfg']['InitialSlidersState'],
            'message' => $message,
        ]);
    }
    ...
    public static function toggleButton($action, $select_name, array $options, $callback)
    {
        ...
        if (true == $options[1]['selected']) {
            $state = 'on';
        } elseif (true == $options[0]['selected']) {
            $state = 'off';
        } else {
            $state = 'on';
        }
 ...
 // end toggleButton()
    ...
    public static function isForeignKeySupported($engine)
    {
        ...
        if (('INNODB' == $engine) || ('PBXT' == $engine)) {
            return true;
        } elseif ('NDBCLUSTER' == $engine || 'NDB' == $engine) {
            $ndbver = strtolower(
                $GLOBALS['dbi']->fetchValue('SELECT @@ndb_version_string')
            );
            if ('ndb-' == substr($ndbver, 0, 4)) {
    ...
    {
        if ('enable' === $GLOBALS['cfg']['DefaultForeignKeyChecks']) {
            return true;
        } elseif ('disable' === $GLOBALS['cfg']['DefaultForeignKeyChecks']) {
            return false;
        }
        return 'ON' == $GLOBALS['dbi']->getVariable('FOREIGN_KEY_CHECKS');
    }
    ...
    public static function handleDisableFKCheckInit()
    {
        $default_fk_check_value
            = 'ON' == $GLOBALS['dbi']->getVariable('FOREIGN_KEY_CHECKS');
    ...
    public static function expandUserString(
        ...
        if (false !== mb_strpos($string, '@FIELDS@')) {
            $string = strtr($string, array('@FIELDS@' => '@COLUMNS@'));
        }

        /* Fetch columns list if required */
        if (false !== mb_strpos($string, '@COLUMNS@')) {
 ...
 // end getSupportedDatatypes()
    ...
    public static function getGISFunctions(
        ...
        if ($display && 'geometry' != $geom_type && 'multipoint' != $geom_type) {
            $funcs[] = array('display' => '--------');
        }
        ...
        } elseif ('multipoint' == $geom_type) {
            // no functions here
        } elseif ('linestring' == $geom_type) {
        ...
        } elseif ('multilinestring' == $geom_type) {
            $funcs['GLength'] = array('params' => 1, 'type' => 'float');
            $funcs['IsClosed'] = array('params' => 1, 'type' => 'int');
        } elseif ('polygon' == $geom_type) {
    ...
    public static function getDefaultFunctionForField(array $field, $insert_mode)
    {
        ...
        if (('timestamp' == $field['True_Type'])
            && $field['first_timestamp']
            && empty($field['Default'])
            && empty($data)
            && 'on update CURRENT_TIMESTAMP' != $field['Extra']
            && 'NO' == $field['Null']
        ) {
        ...
        if ($insert_mode
            && 'PRI' == $field['Key']
            && ('char(36)' == $field['Type'] || 'varchar(36)' == $field['Type'])
        ) {
    ...
    public static function getFunctionsForField(array $field, $insert_mode, array $foreignData)
    {
        ...
        foreach ($functions as $function) {
            $retval .= '<option';
            if (isset($foreignData['foreign_link']) && false !== $foreignData['foreign_link'] && $default_function === $function) {
 ...
 // end getFunctionsForField()
    ...
    public static function getServerSSL()
    {
        ...
        if (!$server['ssl']) {
            ...
            if (!empty($server['socket']) || '127.0.0.1' == $server['host'] || 'localhost' == $server['host']) {
                $class = '';
            }
    ...
    {
        if (empty($value) || 'CURRENT_TIMESTAMP' == $value
    ...
    public static function getCompressionMimeType($file)
    {
        ...
        if ($len >= 3 && 'BZh' == substr($test, 0, 3)) {
            return 'application/bzip2';
        }
        if ($len >= 4 && "PK\003\004" == $test) {
    ...
    public static function isVirtualColumnsSupported()
    {
        ...
        $serverVersion = $GLOBALS['dbi']->getVersion();
        return 'MySQL' == $serverType && $serverVersion >= 50705
             || ('MariaDB' == $serverType && $serverVersion >= 50200);
    }
    ...
    public static function getDbInfo($db, $sub_part)
    {
        ...
        if (empty($tables)) {
            ...
            if (!empty($_REQUEST['tbl_group']) || !empty($_REQUEST['tbl_type'])) {
                ...
                if (isset($sub_part) && '_export' == $sub_part) {
