...
class ExportSql extends ExportPlugin
{
    ...
    protected function setProperties()
    {
        ...
        if ('table' == $plugin_param['export_type']
        ...
        if (!$hide_sql) {
            ...
            if (!$hide_structure) {
                ...
                if ('server' == $plugin_param['export_type']) {
                    $leaf = new BoolPropertyItem(
                        'drop_database',
                ...
                if ('database' == $plugin_param['export_type']) {
    ...
    public function exportHeader()
    {
        ...
        if ((!isset($GLOBALS['sql_compatibility'])
            || 'NONE' == $GLOBALS['sql_compatibility'])
        ) {
        ...
        if (!empty($GLOBALS['asfile'])) {
            ...
            if ('utf8' == $set_names && $GLOBALS['dbi']->getVersion() > 50503) {
                $set_names = 'utf8mb4';
            }
            $head .= $crlf
    ...
    public function exportDBCreate($db, $export_type, $db_alias = '')
    {
        ...
        if ('database' == $export_type && !isset($GLOBALS['sql_create_database'])) {
            return true;
        }
    ...
    public function getTableDef(
        ...
        $warning = '';
        if (false != $result && ($row = $GLOBALS['dbi']->fetchRow($result))) {
            $create_query = $row[1];
            unset($row);
            ...
            if (!$view) {
                ...
                if (('MSSQL' === $compat) || ('"' === $sql_backquotes)) {
            ...
            if (!empty($statement->fields) && (empty($engine) || 'ARCHIVE' != strtoupper($engine))) {
                /**
                ...
                foreach ($statement->fields as $key => $field) {
                    if ($field->isConstraint) {
                        ...
                        // constraints).
                        if ('FULLTEXT KEY' === $field->key->type) {
                ...
                if ((!empty($auto_increment)) && ($update_indexes_increments)) {
                    ...
                    if (isset($GLOBALS['sql_auto_increment'])
                        && (false !== $statement->entityOptions->has('AUTO_INCREMENT'))
                    ) {
    ...
    private function _getTableComments(
        ...
        if ($have_rel) {
            ...
            foreach ($res_rel as $rel_field => $rel) {
                if ('foreign_keys_data' != $rel_field) {
 ...
 // end of the '_getTableComments()' function
    ...
    public function exportData(
        ...
        if (isset($GLOBALS['sql_type'])
            ...
            if ('complete' == $GLOBALS['sql_insert_syntax']
                || 'both' == $GLOBALS['sql_insert_syntax']
            ) {
        ...
        $current_row = 0;
        $query_size = 0;
        if (('extended' == $GLOBALS['sql_insert_syntax']
            || 'both' == $GLOBALS['sql_insert_syntax'])
        ...
        while ($row = $GLOBALS['dbi']->fetchRow($result)) {
            ...
            if (isset($GLOBALS['sql_compatibility'])
                && 'MSSQL' == $GLOBALS['sql_compatibility']
                && 0 == $current_row
            ) {
                if (!Export::outputHandler(
                    'SET IDENTITY_INSERT '
            ...
            for ($j = 0; $j < $fields_cnt; ++$j) {
                ...
                if (!isset($row[$j]) || is_null($row[$j])) {
                    ...
                    // sometimes numeric
                    $values[] = $row[$j];
                } elseif (false !== stristr($field_flags[$j], 'BINARY')
        ...
        if (($statement->options->has('TRIGGER'))
            ...
            for ($i = 0, $count = count($statement->body); $i < $count; ++$i) {
                ...
                if (((Token::TYPE_SYMBOL === $token->type)
                    ...
                    || (((Token::TYPE_KEYWORD === $token->type)
                    && (!($token->flags & Token::FLAG_KEYWORD_RESERVED)))
                    || (Token::TYPE_NONE === $token->type))
                ) {
