...
class Node
{
    ...
    public function parents($self = false, $containers = false, $groups = false)
    {
        $parents = array();
        if ($self
            && (Node::CONTAINER != $this->type || $containers)
        ...
        $parent = $this->parent;
        while (isset($parent)) {
            if ((Node::CONTAINER != $parent->type || $containers)
    ...
    public function hasChildren($count_empty_containers = true)
    {
        ...
        if ($count_empty_containers) {
            ...
            foreach ($this->children as $child) {
                if (Node::OBJECT == $child->type || $child->hasChildren(false)) {
                    $retval = true;
                    break;
                }
            }
    ...
    public function hasSiblings()
    {
        ...
        foreach ($this->parent->children as $child) {
            if ($child !== $this
                && (Node::OBJECT == $child->type || $child->hasChildren(false))
            ) {
                $retval = true;
                break;
            }
