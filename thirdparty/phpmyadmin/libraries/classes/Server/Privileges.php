...
class Privileges
{
    ...
    public static function extractPrivInfo($row = null, $enableHTML = false, $tablePrivs = false)
    {
        ...
        foreach ($grants as $current_grant) {
            if ((!is_null($row) && isset($row[$current_grant[0]]))
                ...
                if ((!is_null($row) && 'Y' == $row[$current_grant[0]])
                    || (is_null($row)
                    && ('Y' == $GLOBALS[$current_grant[0]]
 ...
 // end function
     ...
     */
    public static function getHtmlToDisplayPrivilegesTable(
        $db = '*',
        $table = '*',
        $submit = true
    ) {
            ...
            if ('*' == $table && $GLOBALS['dbi']->isSuperuser()) {
                ...
                while ($row1 = $GLOBALS['dbi']->fetchRow($res)) {
                    if ('max_' == mb_substr($row1[0], 0, 4)) {
                        $row[$row1[0]] = 0;
                    } elseif ('x509_' == mb_substr($row1[0], 0, 5)
                        || 'ssl_' == mb_substr($row1[0], 0, 4)
                    ) {
                        $row[$row1[0]] = '';
                    } else {
                        $row[$row1[0]] = 'N';
 ...
 // end of the 'PMA_displayPrivTable()' function
    ...
    {
        $specified = (isset($row['ssl_type']) && 'SPECIFIED' == $row['ssl_type']);
        $require_options = array(
            ...
            array(
                ...
                'label' => 'REQUIRE SSL',
                'checked' => (
                    isset($row['ssl_type']) && ('ANY' == $row['ssl_type'])
                    ? 'checked="checked"'
                    : ''
                ),
                'disabled' => false,
            ...
            array(
                ...
                'label' => 'REQUIRE X509',
                'checked' => (
                    isset($row['ssl_type']) && ('X509' == $row['ssl_type'])
                    ? 'checked="checked"'
                    : ''
                ),
                'disabled' => false,
    ...
    public static function getHtmlForNotAttachedPrivilegesToTableSpecificColumn(array $row)
    {
        ...
        foreach ($row as $current_grant => $current_grant_value) {
            ...
            if ('Create View_priv' == $current_grant) {
                $tmp_current_grant = 'CreateView_priv';
                $current_grant = 'Create_view_priv';
            } elseif ('Show view_priv' == $current_grant) {
            ...
            $html_output .= '<div class="item">'."\n"
               ...
               .'" value="Y" '
               .('Y' == $current_grant_value ? 'checked="checked" ' : '')
               .'title="';
    ...
    public static function getHtmlForLoginInformationFields(
        ...
        if (isset($GLOBALS['username']) && 0 === strlen($GLOBALS['username'])) {
        ...
        $html_output .= '<option value="any"'
            .((isset($GLOBALS['pred_username']) && 'any' == $GLOBALS['pred_username'])
                ? ' selected="selected"'
                : '').'>'
            .__('Any user')
            .'</option>'."\n";
        ...
        $html_output .= '<option value="userdefined"'
            .((
                !isset($GLOBALS['pred_username'])
                    || 'userdefined' == $GLOBALS['pred_username']
                )
                ? ' selected="selected"'
                : '').'>'
            .__('Use text field')
        ...
        $html_output .= '<option value="any"'
            ...
            .((
                isset($GLOBALS['pred_hostname'])
                    && 'localhost' == $GLOBALS['pred_hostname']
                )
                ? ' selected="selected"'
                : '').'>'
            .__('Local')
        ...
        if (!empty($thishost)) {
            $html_output .= '<option value="thishost"'
                .((
                    isset($GLOBALS['pred_hostname'])
                        && 'thishost' == $GLOBALS['pred_hostname']
                    )
                    ? ' selected="selected"'
                    : '').'>'
                .__('This Host')
        ...
        $html_output .= '<option value="hosttable"'
            .((
                isset($GLOBALS['pred_hostname'])
                    && 'hosttable' == $GLOBALS['pred_hostname']
                )
                ? ' selected="selected"'
                : '').'>'
            .__('Use Host Table')
        ...
        $html_output .= '<option value="userdefined"'
            .((
                isset($GLOBALS['pred_hostname'])
                    && 'userdefined' == $GLOBALS['pred_hostname']
                )
                ? ' selected="selected"'
                : '').'>'
        ...
        $html_output .= '<div class="item">'."\n"
            ...
            .__('Password').'">'."\n"
            .('change' == $mode ? '<option value="keep" selected="selected">'
                .__('Do not change the password')
                .'</option>'."\n" : '')
            .'<option value="none"';

        if (isset($GLOBALS['username']) && 'change' != $mode) {
        ...
        $html_output .= '<div'
            .('sha256_password' != $orig_auth_plugin ? ' class="hide"' : '')
            .' id="ssl_reqd_warning">'
            .Message::notice(
                __(
 ...
 // end of the 'self::getHtmlForLoginInformationFields()' function
    ...
    public static function getUsernameAndHostnameLength()
    {
        ...
        foreach ($fields_info as $val) {
            if ('User' == $val['COLUMN_NAME']) {
                $username_length = $val['CHARACTER_MAXIMUM_LENGTH'];
            } elseif ('Host' == $val['COLUMN_NAME']) {
 ...
 // end of the 'self::getGrants()' function
    ...
    public static function updatePassword($err_url, $username, $hostname)
    {
        ...
        if (empty($message)) {
            ...
            if ('MySQL' == $serverType
                ...
                if ('mysql_old_password' != $authentication_plugin) {
            ...
            } elseif ('MariaDB' == $serverType && $serverVersion >= 10000) {
     ...
     */
    public static function getMessageAndSqlQueryForPrivilegesRevoke(
        $dbname,
        $tablename,
        $username,
        $hostname,
    ...
    public static function getRequireClause()
    {
        ...
        if (isset($arr['ssl_type']) && 'SPECIFIED' == $arr['ssl_type']) {
            $require = array();
        ...
        } elseif (isset($arr['ssl_type']) && 'X509' == $arr['ssl_type']) {
            $require_clause = ' REQUIRE X509';
        } elseif (isset($arr['ssl_type']) && 'ANY' == $arr['ssl_type']) {
    ...
    {
        $sql_query = '';
        if ((isset($_POST['Grant_priv']) && 'Y' == $_POST['Grant_priv'])
            || (isset($GLOBALS['Grant_priv']) && 'Y' == $GLOBALS['Grant_priv'])
        ) {
    ...
    public static function getHtmlListOfPrivs(
        ...
        for ($i = 0; $i < $nbPrivileges; ++$i) {
            ...
            if (isset($current['Table_name'])) {
                ...
                $containsGrant = 'Y' == $current['Grant_priv'];
            }
    ...
    public static function getHtmlForAllTableSpecificRights(
        ...
        foreach ($db_rights as $row) {
            ...
            if ('database' == $type) {
                $name = $row['Db'];
                $onePrivilege['grant'] = 'Y' == $row['Grant_priv'];
            ...
            if ('database' != $type || !empty($row['can_delete'])) {
    ...
    public static function getHtmlTableBodyForUserRights(array $db_rights)
    {
        ...
        foreach ($db_rights as $user) {
            ...
            foreach ($user as $host) {
                ...
                $html_output .= '<td>'
                    .('Y' == $host['Grant_priv'] ? __('Yes') : __('No'))
                    .'</td>'."\n";
    ...
    public static function addUser(
        ...
        $sql = "SELECT '1' FROM `mysql`.`user`"
            ." WHERE `User` = '".$GLOBALS['dbi']->escapeString($username)."'"
            ." AND `Host` = '".$GLOBALS['dbi']->escapeString($hostname)."';";
        if (1 == $GLOBALS['dbi']->fetchValue($sql)) {
    ...
    public static function setProperPasswordHashing($auth_plugin)
    {
        ...
        if ('sha256_password' == $auth_plugin) {
            $GLOBALS['dbi']->tryQuery('SET `old_passwords` = 2');
        } elseif ('mysql_old_password' == $auth_plugin) {
    ...
    public static function getHtmlHeaderForUserProperties(
        ...
        if (!empty($dbname)) {
            ...
            if (!empty($entity_name) && 'table' === $entity_type) {
    ...
    public static function getHtmlForUserOverview($pmaThemeImage, $text_dir)
    {
        ...
        $serverVersion = $GLOBALS['dbi']->getVersion();
        if (('MySQL' == $server_type || 'Percona Server' == $server_type)
            && $serverVersion >= 50706
        ) {
        ...
        if (!$res) {
            ...
            foreach ($db_rights as $right) {
                foreach ($right as $account) {
                    if (empty($account['User']) && 'localhost' == $account['Host']) {
                        $html_output .= Message::notice(
                            __(
     ...
     * @return string $html_output
     */
    public static function getHtmlForUserProperties(
        $dbname_is_wildcard,
        $url_dbname,
        $username,
        $hostname,
        ...
        if (!is_array($dbname) && 0 === strlen($tablename)
        ...
        if (!is_array($dbname) && 0 === strlen($dbname) && !$user_does_not_exists) {
     ...
     */
    public static function getTablePrivsQueriesForChangeOrCopyUser(
        $user_host_condition,
        array $queries,
        $username,
        $hostname
    ...
    public static function getDbSpecificPrivsQueriesForChangeOrCopyUser(
        ...
        while ($row = $GLOBALS['dbi']->fetchAssoc($res)) {
            $queries[] = 'GRANT '.join(', ', self::extractPrivInfo($row))
                ...
                .' TO \''.$GLOBALS['dbi']->escapeString($username)
                .'\'@\''.$GLOBALS['dbi']->escapeString($hostname).'\''
                .('Y' == $row['Grant_priv'] ? ' WITH GRANT OPTION;' : ';');
        }
        $GLOBALS['dbi']->freeResult($res);
    ...
    {
        $serverVersion = $GLOBALS['dbi']->getVersion();
        if (!('MariaDB' == Util::getServerType() && $serverVersion >= 100002)) {
            return false;
        }
    ...
    public static function getSqlQueriesForDisplayAndAddUser($username, $hostname, $password)
    {
        ...
        if (('MySQL' == $serverType || 'Percona Server' == $serverType)
            && $serverVersion >= 50507
            && isset($_POST['authentication_plugin'])
        ) {
        ...
        if ((('MySQL' == $serverType || 'Percona Server' == $serverType)
            ...
            } elseif ('none' == $_POST['pred_password']) {
                $create_user_real = sprintf(
                    $create_user_stmt,
                    null
                );
