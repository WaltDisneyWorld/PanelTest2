...
class UserGroups
{
    ...
    public static function getHtmlForListingUsersofAGroup($userGroup)
    {
        ...
        if ($result) {
            if (0 == $GLOBALS['dbi']->numRows($result)) {
                $html_output .= '<p>'
        ...
        if (null != $userGroup) {
            ...
            if ($result) {
                while ($row = $GLOBALS['dbi']->fetchAssoc($result)) {
                    $key = $row['tab'];
                    $value = $row['allowed'];
                    if ('server_' == substr($key, 0, 7) && 'Y' == $value) {
                        $allowedTabs['server'][] = mb_substr($key, 7);
                    } elseif ('db_' == substr($key, 0, 3) && 'Y' == $value) {
                        $allowedTabs['db'][] = mb_substr($key, 3);
                    } elseif ('table_' == substr($key, 0, 6)
                        && 'Y' == $value
                    ) {
    ...
    public static function edit($userGroup, $new = false)
    {
        ...
        foreach ($tabs as $tabGroupName => $tabGroup) {
            foreach ($tabGroup as $tab => $tabName) {
                ...
                $allowed = isset($_POST[$tabName]) && 'Y' == $_POST[$tabName];
