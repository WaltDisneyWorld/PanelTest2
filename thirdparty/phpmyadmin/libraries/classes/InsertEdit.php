...
class InsertEdit
{
    ...
    public function isColumn(array $column, array $types)
    {
        foreach ($types as $one_type) {
            if (0 === mb_stripos($column['Type'], $one_type)) {
                return true;
            }
        }
        return false;
    ...
    private function getFunctionColumn(
        ...
        $html_output = '';
        if (('blob' === $GLOBALS['cfg']['ProtectBinary']
    ...
    private function getNullifyCodeForNullColumn(
        ...
        if (mb_strstr($column['True_Type'], 'enum')) {
            ...
            && !empty($foreigner)
            && false == $foreignData['foreign_link']
        ) {
            // foreign key in a drop-down
            $nullify_code = '4';
        } elseif (!empty($foreigners)
            && !empty($foreigner)
            && true == $foreignData['foreign_link']
        ) {
    ...
    private function getValueColumn(
        ...
        if (true == $foreignData['foreign_link']) {
    ...
    private function getDropDownDependingOnLength(
        ...
        foreach ($column_enum_values as $enum_value) {
            ...
            if ($data == $enum_value['plain']
                || ('' == $data
                && (!isset($_POST['where_clause']) || 'YES' != $column['Null'])
    ...
    private function getBinaryAndBlobColumn(
        ...
        // Default value : hex
        $fields_type_val = 'hex';
        if (('blob' === $GLOBALS['cfg']['ProtectBinary'] && $column['is_blob'])
            || ('all' === $GLOBALS['cfg']['ProtectBinary'])
            || ('noblob' === $GLOBALS['cfg']['ProtectBinary'] && !$column['is_blob'])
        ) {
    ...
    private function getHtmlInput(
        ...
        } elseif ('date' === $column['True_Type']) {
            $the_class .= ' datefield';
        } elseif ('time' === $column['True_Type']) {
            $the_class .= ' timefield';
        } elseif ('datetime' === $column['True_Type']
            || 'timestamp' === $column['True_Type']
        ) {
        ...
        return '<input type="'.$input_type.'"'
            ...
            : '')
            .($readOnly ? ' readonly="readonly"' : '')
            .(false !== $input_min_max ? ' '.$input_min_max : '')
            .' data-type="'.$data_type.'"'
            .('time' === $input_type ? ' step="1"' : '')
    ...
    private function getValueColumnForOtherDatatypes(
        ...
        $html_output = $backup_field."\n";
        if ($column['is_char']
            && ('textarea' == $GLOBALS['cfg']['CharEditing']
            || false !== mb_strpos($data, "\n"))
        ) {
            $html_output .= "\n";
            ...
            if ('timestamp' == substr($column['pma_type'], 0, 9)) {
                $html_output .= '<input type="hidden" name="fields_type'
                    .$column_name_appendix.'" value="timestamp" />';
            }
            if ('datetime' == substr($column['pma_type'], 0, 8)) {
                $html_output .= '<input type="hidden" name="fields_type'
                    .$column_name_appendix.'" value="datetime" />';
            }
            if ('bit' == $column['True_Type']) {
                $html_output .= '<input type="hidden" name="fields_type'
                    .$column_name_appendix.'" value="bit" />';
            }
            if ('date' == $column['pma_type']
                || 'datetime' == $column['pma_type']
                || 'timestamp' == substr($column['pma_type'], 0, 9)
            ) {
        ...
        if (isset($where_clause)) {
            ...
            if ($found_unique_key && $is_numeric) {
                $html_output .= '<option value="edit_next" '
                    .('edit_next' == $after_insert ? 'selected="selected"' : '').'>'
    ...
    private function getSpecialCharsAndBackupFieldForExistingRow(
        ...
        if (!isset($current_row[$column['Field']])) {
        ...
        } elseif (('timestamp' == substr($column['True_Type'], 0, 9)
            || 'datetime' == $column['True_Type']
            || 'time' == $column['True_Type'])
            && (false !== mb_strpos($current_row[$column['Field']], '.'))
        ) {
            ...
            if ($column['is_binary']
                || ($column['is_blob'] && 'all' !== $GLOBALS['cfg']['ProtectBinary'])
            ) {
                $current_row[$column['Field']] = $as_is
                    ? $current_row[$column['Field']]
                    : bin2hex(
        ...
        if (isset($_POST['default_action'])
            && 'insert' === $_POST['default_action']
        ) {
            if ('PRI' === $column['Key']
                && false !== mb_strpos($column['Extra'], 'auto_increment')
            ) {
    ...
    private function getSpecialCharsAndBackupFieldForInsertingMode(
        ...
        } elseif ('timestamp' == substr($trueType, 0, 9)
            || 'datetime' == $trueType
            || 'time' == $trueType
        ) {
            $special_chars = Util::addMicroseconds($column['Default']);
        } elseif ('binary' == $trueType || 'varbinary' == $trueType) {
    ...
    public function getParamsForUpdateOrInsert()
    {
        if (isset($_POST['where_clause'])) {
            ...
            $is_insert = isset($_POST['submit_type'])
                          && ('insert' == $_POST['submit_type']
                          || 'showinsert' == $_POST['submit_type']
                          || 'insertignore' == $_POST['submit_type']);
        } else {
            // new row => use indexes
            $loop_array = array();
    ...
    public function getGotoInclude($goto_include)
    {
        ...
        if (isset($_POST['after_insert'])
            ...
            if ('db_sql.php' == $GLOBALS['goto'] && strlen($GLOBALS['table']) > 0) {
                $GLOBALS['table'] = '';
            }
        }
        if (!$goto_include) {
    ...
    public function executeSqlQuery(array $url_params, array $query)
    {
        ...
        foreach ($query as $single_query) {
            if ('showinsert' == $_POST['submit_type']) {
    ...
    public function getCurrentValueAsAnArrayForMultipleEdit(
        ...
        if (empty($multi_edit_funcs[$key])) {
            ...
            if ((isset($multi_edit_salt[$key])
                && ('AES_ENCRYPT' == $multi_edit_funcs[$key]
                || 'AES_DECRYPT' == $multi_edit_funcs[$key]))
                || (!empty($multi_edit_salt[$key])
                && ('DES_ENCRYPT' == $multi_edit_funcs[$key]
                || 'DES_DECRYPT' == $multi_edit_funcs[$key]
                || 'ENCRYPT' == $multi_edit_funcs[$key]))
            ) {
                return $multi_edit_funcs[$key].'('.$current_value.",'"
    ...
    public function getCurrentValueForDifferentTypes(
        ...
        if (false !== $possibly_uploaded_val) {
            ...
            if ('protected' != $type && 'set' != $type && 0 === strlen($current_value)) {
                ...
                if ('0x' != substr($current_value, 0, 2)) {
    ...
    public function verifyWhetherValueCanBeTruncatedAndAppendExtraData(
        ...
        if ($row = $this->dbi->fetchRow($result)) {
            $new_value = $row[0];
            if (('timestamp' == substr($meta->type, 0, 9))
                || ('datetime' == $meta->type)
                || ('time' == $meta->type)
            ) {
                $new_value = Util::addMicroseconds($new_value);
            } elseif (false !== mb_strpos($meta->flags, 'binary')) {
    ...
    private function userHasColumnPrivileges(array $table_column, $insert_mode)
    {
        ...
        return ($insert_mode && false !== strstr($privileges, 'insert'))
            || (!$insert_mode && false !== strstr($privileges, 'update'));
    }
}
