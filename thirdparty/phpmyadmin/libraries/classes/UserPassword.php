...
class UserPassword
{
    ...
    public function setChangePasswordMsg()
    {
        ...
        if (('1' != $_POST['nopass'])) {
            if (0 === strlen($_POST['pma_pw']) || 0 === strlen($_POST['pma_pw2'])) {
    ...
    public function changePassword($password, $message, array $change_password_message)
    {
        ...
        if ('MySQL' == $serverType
            ...
            if ('sha256_password' == $orig_auth_plugin) {
                $value = 2;
            } else {
                $value = 0;
            }
    ...
    private function changePassUrlParamsAndSubmitQuery(
        ...
        if ('MySQL' == $serverType && $serverVersion >= 50706) {
            ...
            if ('mysql_native_password' == $orig_auth_plugin) {
                // Set the hashing method used by PASSWORD()
                // to be 'mysql_native_password' type
                $GLOBALS['dbi']->tryQuery('SET old_passwords = 0;');
            } elseif ('sha256_password' == $orig_auth_plugin) {
                // Set the hashing method used by PASSWORD()
