...
class Index
{
    ...
    public static function getFromTableByChoice($table, $schema, $choices = 31)
    {
        ...
        foreach (self::getFromTable($table, $schema) as $index) {
            if (($choices & Index::PRIMARY)
                && 'PRIMARY' == $index->getChoice()
            ) {
                $indexes[] = $index;
            }
            if (($choices & Index::UNIQUE)
                && 'UNIQUE' == $index->getChoice()
            ) {
                $indexes[] = $index;
            }
            if (($choices & Index::INDEX)
                && 'INDEX' == $index->getChoice()
            ) {
                $indexes[] = $index;
            }
            if (($choices & Index::SPATIAL)
                && 'SPATIAL' == $index->getChoice()
            ) {
                $indexes[] = $index;
            }
    ...
    public function generateIndexChoiceSelector($edit_table)
    {
        ...
        foreach (Index::getIndexChoices() as $each_index_choice) {
            if ('PRIMARY' === $each_index_choice
                && 'PRIMARY' !== $this->_choice
