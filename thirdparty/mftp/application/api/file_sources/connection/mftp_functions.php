<?php
    ...
    function mftp_get_systype($connection)
    {
        ...
        if (215 != $resp->code && 200 != $resp->code) {
    ...
    function mftp_chdir($connection, $directory)
    {
        ...
        if (250 != $resp->code && 200 != $resp->code) {
    ...
    function mftp_pwd($connection)
    {
        ...
        if (257 != $resp->code && 200 != $resp->code) {
    ...
    function mftp_rawlist($connection, $listArgs)
    {
        ...
        if (226 == $resp->code) { // some servers don't open a ftp-data connection if the directory is empty
            return array();
        }

        if (150 != $resp->code && 125 != $resp->code) {
    ...
    function mftp_put($connection, $remotePath, $localPath, $transferMode, $progressCallbackTimeout = null, $progressCallback = null)
    {
        ...
        while ($buf = fread($handle, MFTP_BUFF_SIZE)) {
            ...
            if (null !== $progressCallbackTimeout && null !== $progressCallback) {
    ...
    function _mftp_get_transfer_mode_code($transferMode)
    {
        ...
        return FTP_ASCII == $transferMode ? 'A' : 'I';
    }
    ...
    function _mftp_pasv_setup($connection)
    {
        ...
        if (227 != $resp->code && 200 != $resp->code) {
    ...
    function _mftp_epsv_setup($connection)
    {
        ...
        if (229 != $resp->code && 200 != $resp->code) {
    ...
    function _mftp_put_cmd($connection, $command, $args = null)
    {
        ...
        if (false === $bytesWritten || $bytesWritten != strlen($command)) {
    ...
    function _mftp_read_last_line($connection, $socket)
    {
        ...
        return 0 == $lineCount ? null : $lines[$lineCount - 1];
    }
    ...
    function _mftp_convert_response_to_exception($message, $response, $path = null)
    {
        ...
        if (552 == $response->code) {
            throw new MFTPRemoteFileException('Quota exceeded.', $response->code, $path);
        }

        if (550 == $response->code || 553 == $response->code || 451 == $response->code) {
            if (451 == $response->code || false !== strpos($normalisedResponseText, 'permission denied')) {
            ...
            if (false !== strpos($normalisedResponseText, 'file exists')) {
            ...
            if (550 == $response->code || false !== strpos($normalisedResponseText, 'no such file')
                || false !== strpos($normalisedResponseText, "doesn't exist")
                || false !== strpos($normalisedResponseText, 'failed to change directory') // this is a guess
                || false !== strpos($normalisedResponseText, 'folder not found')
                || false !== strpos($normalisedResponseText, 'not find')
                || false !== strpos($normalisedResponseText, 'not found')
            ) {
    ...
    {
        $ipVersionLookup = false === strpos($localIp, ':') ? '1' : '2';  // 1 = v4, 2 = v6
