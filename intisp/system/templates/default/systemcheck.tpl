...
<?php 
require 'include/head.php';onlymasterreseller();
?>
    <div class="content-wrapper">
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-md-12">
<h1 class="page-title">System Checklist</h1>
<p>Shows recommendations for PHP, Apache2, IntISP, and Firewall.</p>
<?php
...
test_debug_build();
function test_webister_config() {
	$meta = tdesc("Configuration", "Checks if IntISP Configuration is secure.");
		    $p = substr(sprintf('%o', fileperms('config.php')), -4);
if ($p == "0444") {
	
} else {
		tres($meta, TEST_MEDIUM, "IntISP Configuration", "IntISP permission level set to $p. The configuration file does not have a permission level of 0444.");
}
}
test_webister_config();
// got root?
...
<div class="c">
  ...
  <tbody>
	...
	foreach ($all_result_codes as $sev) {
		...
		<tr class="alert alert-<?php
if ($res['result'] == "high" || $res["result"] == "critical") {
			echo 'danger';
		}
		else if ($res['result'] == "medium" || $res["result"] == "maybe") {
			echo 'warning';
		}
		...
		if ($res['result'] == "high" || $res["result"] == "critical") {
			echo '<i class="fa fa-times-circle" aria-hidden="true"></i> Danger';
		}
		else if ($res['result'] == "medium" || $res["result"] == "maybe") {
			echo '<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Warning';
		}
		else if ($res['result'] == "low") {
			echo '<i class="fa fa-info-circle" aria-hidden="true"></i> Low';
		} else {
			echo '<i class="fa fa-info-circle" aria-hidden="true"></i> Other';
		}
		?></td>
