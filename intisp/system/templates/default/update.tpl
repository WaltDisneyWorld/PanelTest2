...
if (version_compare(phpversion(), '4.1') < 0)
  {
    redodie(print_message(i18n('ERROR'), i18n('PHP 4.1 or greater is required.'), $color = 'red'));
  }
@set_time_limit(0);
...
if ($getVersions != '' and $currentVersion != '')
  {
        ...
        if ($actualVersion > $currentVersion)
          {
            ...
            if (isset($_GET['doUpdate']) and $_GET['doUpdate'] == true)
            ...
            else
              {
                ...
                print_message('', '&raquo; <a href="?page=update&doUpdate=true">' . i18n('Install Now?') . '</a>');
              }
            break;
          }
      }
    if (isset($updated) and $updated == true)
    ...
    else if (!isset($found) or $found != true)
      {
        ...
        print_message('', '&raquo; <a href="?page=update&check_for_updates=true">' . i18n('Check For Updates?') . '</a>');
      }
  }
