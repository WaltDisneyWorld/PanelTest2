...
function sftp_connect($host, $username, $pass, $protocol = 'ftp', $port = 21, $cd = false, $pri = '', $passphrase = ''){
	...
	if(!class_exists($protocol) && file_exists($globals['mainfiles'].'/classes/'.$protocol.'.php')){
		include_once($globals['mainfiles'].'/classes/'.$protocol.'.php');
	}
	...
	if(empty($ftp)){
		...
		// Initialize a Class
		if($protocol == 'customio' && file_exists($globals['mainfiles'].'/classes/customio.php')){
...
$ftp = sftp_connect($__settings['server_host'], $__settings['ftp_user'], $__settings['ftp_pass'], $__settings['protocol'], $__settings['port'], $__settings['ftp_path'], $__settings['private_key'], $__settings['passphrase']);

if(!is_object($ftp)){
	$error[] = 'Could not connect to the source server using '.$__settings['protocol'];
