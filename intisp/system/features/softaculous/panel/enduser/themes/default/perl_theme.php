...
if((empty($globals['default_protocol']) || $globals['default_protocol'] <= 1) && empty($globals['off_checkhttps'])){
	...
	function check_https(){
		$.ajax({
			type: "POST",
			url: "'.$globals['index'].'act=perl&soft='.$soft.'&checkhttps="+encodeURIComponent("https://"+$("#softdomain").val()),
			timeout:10000,
			...
			foreach($sets as $sk => $sv){
				...
				<div class="row">
					...
					'.(preg_match('/admin_pass|softpass/is', $sv['tag']) ? preg_replace('/>|\/>/is',' onkeyup="check_pass_strength();" >', '
						...
						<div class="col-sm-1"><div class="row" style="margin-left:-25px;"><div class="col-sm-5"><input id="toggle_passwd" type="checkbox" style="display:none;" onclick="toggle_pass(\'show_hide\', \'admin_pass\');"/><label for="toggle_passwd" style="margin-top:6px;"><span id="show_hide">Hide</span></label></div><div class="col-sm-1"></div>
						<div class="col-sm-6"><a href="javascript: void(0);"  onclick="$_(\''.$sk.'\').value=randstr(10, 1, '.(!empty($globals['pass_strength']) ? $globals['pass_strength'] : 0).');check_pass_strength();return false;" title="'.$l['randpass'].'"><img src="'.$theme['images'].'randpass.gif" /></a>
						</div></div></div>
