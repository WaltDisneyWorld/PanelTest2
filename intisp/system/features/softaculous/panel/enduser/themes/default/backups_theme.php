...
if(!empty($globals['max_backups'])){
	echo '<center class="alert alert-warning"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>'.lang_vars($l['max_backups'], array($cur_backups, $globals['max_backups'])).'</center>';
}
				...
				<tr>
					...
					foreach($backs as $bk => $bv){
						...
						foreach($bv as $ik => $iv){
							...
							<td width="4%">
								<a href="'.$globals['ind'].'act=backups&remove='.rawurlencode($iv['name']).(!empty($info_only) ? '&info_only=1' : '').'" title="'.$l['remove'].'" onclick="return _confirm();"><img src="'.$theme['images'].'remove.gif" /></a>
							</td>
							<td width="3%">
