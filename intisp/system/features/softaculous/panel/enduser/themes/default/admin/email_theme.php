...
<div class="container-fluid width80"><br /><br />
	...
	if(!empty($saved)){
		echo '<div class="alert alert-success text-center"><a href="#" class="close" data-dismiss="alert">&times;</a>
		<img src="'.$theme['images'].'success.gif" /> '.$l['settings_saved'].'</div>';
	}
	
	if(!empty($globals['enc_mail_pass']) && !empty($globals['mail_pass'])){
		$globals['mail_pass'] = pass_decrypt($globals['mail_pass']);
	}
	...
	<div class="row">
		...
		<div class="col-sm-1" style="margin-left:-20px;">
			<input id="toggle_pass_mail_pass" type="checkbox" style="display:none;" onclick="toggle_pass(\'show_hide_mail_pass\', \'mail_pass\');"/><label for="toggle_pass_mail_pass" style="margin-top:6px; cursor:pointer;"><span id="show_hide_mail_pass">Show</span></label>
		</div>
	</div><br />
	...
	<p align="center"><input type="submit" name="editemailsettings" class="flat-butt" value="'.$l['edit_settings'].'"/></p>
	</div>
</div>
</form>';
