				...
				success: function(data){
					...
					if(filenames.length == file_count){
						if(not_deleted_files != ""){	
							...
							if(remote_flag == 0){
								div = "<div class=\'alert alert-success fade in\' style=\'width:95% ; padding:10px; font-size:15px;\'><a href=\'#\' class=\'close\' data-dismiss=\'alert\'>&times;</a><center>'.$l['selected_files_deleted'].'</center>";
							}else{
								div = "<div class=\'alert alert-success fade in\' style=\'width:95% ; padding:10px; font-size:15px;\'><a href=\'#\' class=\'close\' data-dismiss=\'alert\'>&times;</a><center>'.$l['removing_remote_files'].'</center>";
							}
							$("#msg_backup_"+type).html(div);
						}
			...
			success: function(result){
				if(result == "1"){				
					...
					if($("#backup_server_table").children().length == 0){
						$("#backup_server_table").html("<tr><td colspan=7><center><span>'.$l['no_backup_servers'].'</span></center></td></<tr>");
					}					
				}
			},
			error: function(){
...
function show_table($type, $file_type){
	...
	if(empty($wbackup_list[$type])){
		echo '<tr><td colspan=6><center><span>'.$l['no_backup'].' !</span></center></td></<tr>';
	}else{
...
function list_autobackups(){
	...
	<table border="0" cellpadding="8" cellspacing="1" width="100%" align="center" class="table table-hover table-hover-moz td_font">			
		...
		if(empty($list_autobackups)){
			echo '<tr><td colspan=7><center><span>'.$l['no_autobackup'].'</span></center></td></<tr>';
		}else{
...
function show_remote_backup_servers($type){
	...
	echo '<table border="0" cellpadding="6" cellspacing="1" width="100%" class="table table-hover table-hover-moz" align="center">
			...
			if(empty($backup_servers)){
				echo '<tr><td colspan=7><center><span>'.$l['no_backup_servers'].'</span></center></td></<tr>';
			}else{
				foreach($backup_servers as $key => $value){
					echo '
						<tr id="'.$key.'">
