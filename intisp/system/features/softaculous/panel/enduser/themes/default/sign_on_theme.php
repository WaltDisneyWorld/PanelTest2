...
global $insid, $user, $autoid, $__settings, $globals, $software, $iscripts, $soft, $theme, $sign_on_url, $error, $l, $SESS;
	...
	if(empty($error)){
		...
		soft_shell_exec_bg(phpbin().' '.to_win_path($globals['path']).'/cli.php --clean_sign_on --insid='.$insid.' --file='.$autoid.(aefer() || ent() ? ' --username='.$SESS['username'].' --owner='.$SESS['owner'] : ($globals['softpanel'] == 'hc' ? ' --username='.$dom : '')));
