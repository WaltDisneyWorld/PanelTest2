	...
	if(empty($softpanel->noprotocol) && empty($globals['hide_protocol'])){
		...
		<div class="row custom_install">
			...
			<div class="col-sm-7">
				...
				<div class="row">
					<span id="httpserror" style="display:none; padding:10px;" class="alert alert-warning"></span>
				</div>
			</div>
		</div>';
	}
						...
						<div class="script_fields" '.(!empty($sv['quick_install']) ? 'quick_install="1"' : '').'>
							<div class="row">
								...
								'.(preg_match('/admin_pass|softpass/is', $sv['tag']) ? preg_replace('/>|\/>/is',' onkeyup="check_pass_strength();">', '
									...
									<div class="col-sm-1"><div class="row" style="margin-left:-25px;"><div class="col-sm-5"><input id="toggle_pass_admin" type="checkbox" style="display:none;" onclick="toggle_pass(\'show_hide_admin\', \'admin_pass\');"/><label for="toggle_pass_admin" style="margin-top:6px;"><span id="show_hide_admin">Hide</span></label></div><div class="col-sm-1"></div>
										<div class="col-sm-6"><a href="javascript: void(0);"  onclick="$_(\''.$sk.'\').value=randstr(10, 1, '.(!empty($globals['pass_strength']) ? $globals['pass_strength'] : 0).');check_pass_strength();return false;" title="'.$l['randpass'].'"><img src="'.$theme['images'].'randpass.gif" /></a></div></div>
									</div>
	...
	<div class="bg custom_install">
		<div class="sai_sub_head" id="advoptions_toggle" onclick="toggle_advoptions(\'advoptions\');" style="cursor:pointer"><img id="advoptions_toggle_plus"  src="'.$theme['images'].'plus_new.gif" style="margin-top:-4px;"/>&nbsp;&nbsp;'.$l['adv_option'].'</div>
			<div id="advoptions" style="display:none;"><hr>
				...
				'.(empty($dbtype) || aefer() || !empty($softpanel->auto_managedb) ? "" :
				'<div class="row">
					<div class="col-sm-5">
						<label for="softdb" class="sai_head">'.$l['database_name'].'</label><br />
					...
					<div class="col-sm-7">
						<input type="text" name="softdb" class="form-control" id="softdb" size="30" value="'.POSTval('softdb', $dbdetails['dbname']).'" onblur="checkdbname(\'softdb\', false)" autocomplete="false" /><br />
						<span id="softdberror" style="background: #FDB3B3; display:none; width:200px;"></span>
					</div>
				</div>');
	...
	<div class="bg2">
		...
		foreach($software['branch'] as $csid => $cver){
			...
			foreach($installations[$csid] as $ik => $iv){
				...
				(!empty($iv['eu_auto_upgrade']) && empty($no_autoupgrade['disable_autoupgrade']) &&  empty($no_autoupgrade['scripts'][$csid]) && !empty($scripts[$iv['sid']]['auto_upgrade']) && empty($no_autoupgrade_acl)? '<img src="'.$theme['images'].'auto_upgrade.png"  title="'.$l['auto_upgrade_enabled'].'" style="cursor:pointer"/>' : '').'</td>'.				
				(!empty($is_cloneable) ? '<td width="25" class="someclass" title="'.$l['clone'].'"><a href="'.$globals['ind'].'act=sclone&insid='.$ik.'" ><img src="'.$theme['images'].'clone.gif" style="min-width: 20px;" /></a></td>&nbsp;' : '').
				(empty($globals['disable_backup_restore']) ? '<td width="25" title="'.$l['backup'].'" class="someclass"><a href="'.$globals['ind'].'act=backup&insid='.$ik.'"><img src="'.$theme['images'].'backup.gif" style="min-width: 20px;" /></a></td>' : '').'
				&nbsp;<td width="25" title="'.$l['editdetail'].'" class="someclass"><a href="'.$globals['ind'].'act=editdetail&insid='.$ik.'"><img src="'.$theme['images'].'editdetail.gif" style="min-width: 20px;" /></a></td>
				'.(asperapp(0, 0, 1) && !empty($can_publish) ? ' &nbsp;<td><a href="'.$globals['ind'].'act=publish&insid='.$ik.'" title="'.$l['publish'].'"><img src="'.$theme['images'].'publish.gif" style="min-width: 20px;" /></a></td>' : '').' 
				&nbsp;<td width="25" title="'.$l['remove'].'" class="someclass"><a href="'.$globals['ind'].'act=remove&insid='.$ik.'"><img src="'.$theme['images'].'remove.gif" style="min-width: 20px;" /></a></td>'.				
