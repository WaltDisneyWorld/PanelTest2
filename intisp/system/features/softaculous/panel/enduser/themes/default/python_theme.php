...
if((empty($globals['default_protocol']) || $globals['default_protocol'] <= 1) && empty($globals['off_checkhttps'])){
	...
	function check_https(){
		$.ajax({
			type: "POST",
			url: "'.$globals['index'].'act=python&soft='.$soft.'&checkhttps="+encodeURIComponent("https://"+$("#softdomain").val()),
			timeout:10000,
...
<div class="col-sm-7">
	<select name="softproto" id="softproto" onblur="checkhttps(\'softproto\', \'softdomain\', false)" class="form-control">';
	...
	<span id="httpserror" style="background: #FDB3B3; display:none; width:200px;" class="alert alert-warning"></span>
</div>
</div>';
...
$info['install'] .= '<div class="row">
<div class="col-sm-5">
	<label class="sai_head" for="softdomain">'.$l['choose_domain'].'</label><br />
	<span class="sai_exp2">'.$l['choose_domain_exp'].'</span>
</div>
<div class="col-sm-7">
	<select name="softdomain" id="softdomain" onchange="softmail();change_admin_prefix(this.value);" onblur="checkhttps(\'softproto\', \'softdomain\', false)" class="form-control">';
...
<div class="row">
<div class="col-sm-5">
	<label class="sai_head" for="softdirectory">'.$l['in_directory'].'</label><br />
	<span class="sai_exp2">'.$l['in_directory_exp'].'</span>
</div>
<div class="col-sm-7">
	<input type="text" name="softdirectory" id="softdirectory" value="'.POSTval('softdirectory', (!empty($globals['no_prefill']) ? '' : $software['softname'])).'" class="form-control" />
</div>
</div><br/>';
...
<div class="col-sm-2"><div class="row" style="text-align:center;"><div class="col-sm-5"><input id="toggle_passwd" type="checkbox" style="display:none;" onclick="toggle_pass(\'show_hide\', \'admin_pass\');" /><label for="toggle_passwd" style="margin-top:6px;"><span id="show_hide">Hide</span></label></div><div class="col-sm-7"><a href="javascript: void(0);" onclick="$_(\''.$sk.'\').value=randstr(10, 1, '.(!empty($globals['pass_strength']) ? $globals['pass_strength'] : 0).');check_pass_strength();return false;" title="'.$l['randpass'].'"><img src="'.$theme['images'].'randpass.gif" /></a></div></div></div></div>
