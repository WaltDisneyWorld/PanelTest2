...
global $software, $soft, $info, $settings, $init_tab, $installed, $removed, $__settings, $notes, $cron, $overwrite_option, $ampps_package, $nopackage, $install, $ampps_apps_installed, $currently_ruunning, $isMysql55installed, $show_warning_note, $info_path, $softinfo;
...
if(empty($globals['lictype']) && !empty($a_apps[$soft]['force_apps'])){
	$info['overview'] .= '<center class="sai_anotice">'.lang_vars($l['not_in_free'], array($a_apps[$soft]['fullname'])).'</center><br />';
}
...
';
	...
	//Add Install and Remove Button.

	if(!empty($software['default'])){
		$info['overview'] .= '<center><div class="alert alert-warning">
		<span class="sai_exp2"><strong>'.$l['note'].'</strong>&nbsp'.$l['software_note'].'</span></div></center>';
	}else if(!empty($currently_ruunning)){
		$info['overview'] .= '<center><div class="alert alert-warning">
		<span class="sai_exp2"><strong>'.$l['note'].'</strong>&nbsp'.$l['notes_remove_php'].'</span></div></center>';
		...
		if($software['type'] == 'database'){
			$info['overview'] .= '<center><div class="alert alert-danger">
		<span class="sai_exp2"><strong>'.$l['note'].'</strong>&nbsp'.$l['note_remove'].'</span></div></center>';
		}				
...
$info['notes'] = '<br>
	<table border="0" cellspacing="3" cellpadding="10" align="center" class="notestable">
		<tr style="height: 30px;">
			<th bgcolor="#EFEFEF" align="center">&nbsp&nbsp&nbsp&nbsp'.$l['desc'].'</th>
			<th bgcolor="#EFEFEF" align="center">&nbsp&nbsp&nbsp&nbsp'.$l['c_path'].'</th>
		</tr>
		<tr style="height: 30px;">				
			<td bgcolor="#E6F5FF">&nbsp&nbsp&nbsp&nbsp'.$l['binary'].'</td>
			<td bgcolor="#E6F5FF">&nbsp&nbsp&nbsp&nbsp'.$globals['ampps_path']. DIRECTORY_SEPARATOR .$software['softname'].$software['bin'].'&nbsp&nbsp<td>		
		</tr>
		<tr style="height: 30px;">				
			<td bgcolor="#E6F5FF">&nbsp&nbsp&nbsp&nbsp'.$l['conf'].'</td>
			<td bgcolor="#E6F5FF">&nbsp&nbsp&nbsp&nbsp'.$globals['ampps_path']. DIRECTORY_SEPARATOR .$software['softname'].$software['conf_path'].'&nbsp&nbsp&nbsp&nbsp</td>
			</tr>
	</table><br>
';
